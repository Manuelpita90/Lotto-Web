...
const selectSorteo = document.getElementById('last-result');
...
btnPredecir.disabled = true; // Iniciar deshabilitado
selectSorteo.addEventListener('change', () => {
    btnPredecir.disabled = !selectSorteo.value;
    localStorage.setItem('lotto_last_animal', selectSorteo.value);
});
...
function abrirModalManual() {
    // Copiamos las opciones del select principal si el manual está vacío
    if (selectManualAnimal.options.length <= 1) {
        selectManualAnimal.innerHTML = selectSorteo.innerHTML;
        selectManualAnimal.value = ""; // Resetear selección
    }
...
async function predecir() {
    resultsArea.innerHTML = `
        <div class="loading-text" style="display:block">
            <i class="fas fa-circle-notch fa-spin"></i> Analizando patrones en BD...
        </div>
    `;
    ...
    try {
        ...
        resultsArea.innerHTML = `<div style="text-align:center; color:#ff5555; padding:20px;">Error: ${error}</div>`;
    }
}
    ...
    if (!lista || lista.length === 0) {
        resultsArea.innerHTML = `
            <div class="empty-state">
                ...
                <p><b>Insuficientes datos</b></p>
                <small>La IA necesita aprender patrones. Guarda una secuencia (ej: Pavo -> Carnero -> Pavo) para ver predicciones.</small>
            </div>`;
        ultimasPrediccionesNombres = [];
        actualizarHighlightsHoy(); // Limpiar resaltados si no hay predicción
        return;
    }
...
async function actualizarResultadosHoy() {
    ...
    try {
        ...
        // Auto-seleccionar el último resultado en el input
        ...
        if (selectSorteo.value !== valSelect) {
            selectSorteo.value = valSelect;
            btnPredecir.disabled = false;
            localStorage.setItem('lotto_last_animal', valSelect);
            // Feedback visual (Brillo verde temporal)
            selectSorteo.style.boxShadow = "0 0 15px var(--neon-green)";
            setTimeout(() => selectSorteo.style.boxShadow = "", 800);
...
(function init() {
    // 1. Restaurar último animal seleccionado
    const savedAnimal = localStorage.getItem('lotto_last_animal');
    if (savedAnimal) {
        selectSorteo.value = savedAnimal;
        btnPredecir.disabled = false;
    }
